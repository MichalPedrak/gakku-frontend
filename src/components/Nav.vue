<template>
  <nav class="vh-100 d-flex flex-column flex-shrink-0 p-3 text-white bg-dark" style="width: 280px;">
    <a href="/" class="w-100 d-flex align-items-center mb-3 mb-md-0 me-md-auto text-white text-decoration-none">
      <svg class="bi me-2" width="40" height="32"><use xlink:href="#bootstrap"></use></svg>
      <span class="fs-4">Sidebar</span>
    </a>
    <hr>
    <ul class="w-100 nav nav-pills flex-column mb-auto text-start">
      <li class="nav-item">
        <router-link class="nav-link text-white" to="/">Panel główny</router-link>
      </li>

      <li>
        <router-link class="nav-link text-white" to="/users">Użytkownicy</router-link>
      </li>
      <li>
        <router-link class="nav-link text-white" to="/fiszki">Fiszki</router-link>
      </li>

      <li @click="logout" >
        <a href="#" class="nav-link text-white">
          <svg class="bi me-2" width="16" height="16"><use xlink:href="#people-circle"></use></svg>
        Wyloguj się
        </a>
      </li>
    </ul>
    <hr>
    <div class="dropdown">
      <a href="#" class="d-flex align-items-center text-white text-decoration-none dropdown-toggle" id="dropdownUser1" data-bs-toggle="dropdown" aria-expanded="false">
        <img src="https://github.com/mdo.png" alt="" width="32" height="32" class="rounded-circle me-2">
        <strong>{{ auth.name }}</strong>
      </a>
      <ul class="dropdown-menu dropdown-menu-dark text-small shadow" aria-labelledby="dropdownUser1">
        <RouterLink to="/">Home</RouterLink>
        <RouterLink to="/login">Login</RouterLink>
        <RouterLink to="/register">Register</RouterLink>
        <RouterLink to="/register">Register</RouterLink>
      </ul>
    </div>
  </nav>

</template>

<script>
import { useAuthStore } from '../store/index.js'
import {useRouter} from "vue-router";
import {reactive, ref} from "vue";


export default {
  name: "Nav",
  setup(){

    const router = useRouter();
    const store = useAuthStore();
    const auth = reactive(JSON.parse(store.user))
    // const auth = store.user

    const name = "test"

    const logout = async () => {
        await fetch('http://localhost:8000/api/logout', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          credentials: 'include',
        })

        await store.logout()
        await router.push('/login');
      };

    return{
      auth,
      name,
      logout
    }
  }
}
</script>

<style scoped>
  nav{
    display: flex;
    align-items: flex-start;
  }
</style>
